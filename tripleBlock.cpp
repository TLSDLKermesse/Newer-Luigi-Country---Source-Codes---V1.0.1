...
Profile TripleBlockProfile(&daEnTripleBlock_c::build, SpriteId::502, &TripleBlockSpriteData, ProfileId::, ProfileId::, "TripleBlock", TripleBlockFileList);
...
void daEnTripleBlock_c::blockWasHit(bool isDown) {
	...
	static u32 reggieToIG[] = {0x0,0x1,0x2,0x7,0x9,0xE,0x11,0x15,0x19,0x6,0x2,1x0,1x1,1x2,1x3,1x4,1x5,1x6,1x7,1x8,1x9,1x10,1x11};
	//Mushroom, Star, Coin, 1UP, Fire Flower, Ice Flower, Penguin, Propeller, Mini Shroom, Hammer, 10 Coins, Breaks Itself or spawns ennemies :OOOOOOOOO Pretty interesting isn't it?
	u32 powerupToSet = reggieToIG[(this->settings >> 24 & 0xFF)]; //Spritedata ID to EN_ITEM ID
	...
	if((powerupToSet != 0x2 && powerupToSet != 0x15 && powerupToSet != 1x3) || p == 0 || p == 3) { //Propeller does a different sound and Coin doesn't do any sound
		PlaySoundWithFunctionB4(SoundRelatedClass, &handle, SE_OBJ_ITEM_APPEAR, 1); //Item sound
	} else if (powerupToSet == 1x3) { //Is the powerup a mega mushroom
		extern "C" bool SpawnEffect(const char *BlockSpawnSound [1], int, Vec*, S16Vec*, Vec*);
	}
	...
	if((this->settings >> 24 & 0xFF) == 10) { //If 10 Coins variant, then only add 0.1 to wasIHit
		this->wasIHit += 0.1f;
	}
...
const char* BlockSpawnSound[2] = {
	"sfx/ACORN_SPAWN" //As I'm tired of inventing names :|
	"sfx/BIG_KINOKO_SPAWN"
}